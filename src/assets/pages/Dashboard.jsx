      ...
      <TableRow>
        <TableCell padding="checkbox">
          <Checkbox
            ...
            inputProps={{
              "aria-label": "select all fraccionamientos",
            }}
          />
        </TableCell>
...
const FraccionamientoTableToolbar = (props) => {
...
FraccionamientoTableToolbar.propTypes = {
  numSelected: PropTypes.number.isRequired,
};

export default function FraccionamientoTable({ fraccId }) {
  ...
  useEffect(() => {
    const fetchFraccionamiento = async () => {
      try {
        const response = await fetch(`${API_URL}/api/fracc/${fraccId}`);
        const data = await response.json();
        setRows([data]);
      } catch (error) {
        console.error("Error fetching fraccionamiento:", error);
      }
    };
  ...
  return (
    <Box sx={{ width: "100%" }}>
      <Paper sx={{ width: "100%", mb: 2 }}>
        <FraccionamientoTableToolbar numSelected={selected.length} />
        <TableContainer>
          <Table
            sx={{ minWidth: 750 }}
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell align="right">
                        {row.residencias ? row.residencias.length : 0}
                      </TableCell>
                    </TableRow>
                  );
                })}
